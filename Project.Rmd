...
ggplot(rings_5_15, aes(x = rings_5_15$Diameter, fill = factor(Rings))) +
  ...
  ggtitle("График плотностей распределения по кольцам")
...
Чтобы проводить тест, посмотрим, нормально ли распределены данные с помощью теста Шапиро-Вилка:

```{r}
shapiro.test(data_without_NA$Diameter)
```

Судя по результату, данные имеют не нормальное распределение, поэтому проверять свои гипотезы будем с помощью теста Манна-Уитни.

```{r}
wilcox.test(Diameter ~ Rings, data = data_without_NA, subset = Rings %in% c(5, 15))
```

Отвергаем гипотезу о равенстве средних для выборок с 5ю кольцами и 15ю (p-value < `r wilcox.test(Diameter ~ Rings, data = data_without_NA, subset = Rings %in% c(5, 15))$p.value`).
...
с p-value около `r cor.test(data_without_NA$Diameter, (data_without_NA$Whole_weight) ** (1/3))$p.value` отвергаем H0-гипотезу о том, что они не скоррелированы. Нарисуем график:
...
```{r}
shapiro.test(data_without_NA$Rings)
```

Данные не являются нормальными, поэтому проведем тесты Манна-Уитни:
...
### 11. Построение множественной регрессионной модели для значения веса моллюска.

Есть веские основания полагать, что общий вес моллюска складывается из веса раковины, веса внутренностей и веса почищенного моллюска. Построим линейную модель, описывающую "веса весов в общем весе" для моллюска, то есть какая часть от веса моллюска вносит бОльший вклад в общий вес моллюска. 

```{r}
weight.lm <- lm(Whole_weight ~ Shucked_weight+Viscera_weight+Shell_weight, data_without_NA)
summary(weight.lm)
```

В построенной модели, во первых, мы видим интерсепт чуть меньше нуля (конечно, хотелось бы увидеть здесь ноль, но это тоже не так уж и плохо). Во-вторых, можем сделать вывод, что для линейного приближения, вес раковины вносит наибольший вклад в общий вес моллюска, но в целом незначительно больше. При этом, наша модель объясняет 99% дисперсии, что говорит о том, что такая модель имеет довольно хорошую предсказательную силу (в интерполяционном смысле). Если смотреть на остатки, то в целом они не имеют нормальное распределение, но в серединке интервала можем считать, что они нормальны. 

```{r}
plot(data_without_NA$Whole_weight, resid(weight.lm), ylab="Residuals", xlab="Whole weight") 
abline(0, 0)  
```